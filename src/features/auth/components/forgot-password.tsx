...
const ForgotPassword: React.FC<ForgotPasswordProps> = ({ switchFlow }) => {
  ...
  return step === "forgot" ? (
    <Card className='md:relative w-full  border-none md:border-solid md:border-2 border-black bg-salYellow md:bg-white shadow-none  p-8'>
      ...
      <Form {...forgotForm}>
        <CardHeader className='px-0 pt-0 space-y-6 items-center justify-center'>
      ...
      <Form {...resetForm}>
        ...
        <CardContent className='p-0'>
          {success === "Code sent!" && !error && (
            <div className='w-auto flex md:mx-10 gap-x-4 items-center justify-center p-4  rounded-lg bg-emerald-500/20'>
          ...
          <form
            ...
            {resetErrors.code && (
              <p className='text-red-500 text-sm'>{resetErrors.code.message}</p>
            )}
            {/* <Input
              {...resetForm.register("code")}
              placeholder='Code'
